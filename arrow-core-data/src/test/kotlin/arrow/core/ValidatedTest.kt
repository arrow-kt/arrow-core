...
class ValidatedTest : UnitSpec() {
  ...
  init {
    ...
    testLaws(
      ...
      FxLaws.laws<ValidatedPartialOf<String>, Int>(Gen.int().map(::Valid), Gen.validated(Gen.string(), Gen.int()).map { it }, Validated.eqK(String.eq()).liftEq(Int.eq()), Validated.Companion::fxEager, Validated.Companion::fx)
    )
