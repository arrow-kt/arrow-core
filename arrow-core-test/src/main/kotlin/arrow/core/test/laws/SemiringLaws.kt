...
object SemiringLaws {
  ...
  // a + b = b + a
  private suspend fun <F> Semiring<F>.semiringAdditiveCommutativity(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN, GEN) { a, b ->
      a.combine(b).equalUnderTheLaw(b.combine(a), EQ)
    }

  // 0 + a = a
  private suspend fun <F> Semiring<F>.semiringAdditiveLeftIdentity(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN) { A ->
      (zero().combine(A)).equalUnderTheLaw(A, EQ)
    }

  // a + 0 = a
  private suspend fun <F> Semiring<F>.semiringAdditiveRightIdentity(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN) { A ->
      A.combine(zero()).equalUnderTheLaw(A, EQ)
    }

  // a + (b + c) = (a + b) + c
  private suspend fun <F> Semiring<F>.semiringAdditiveAssociativity(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN, GEN, GEN) { A, B, C ->
      A.combine(B.combine(C)).equalUnderTheLaw((A.combine(B)).combine(C), EQ)
    }

  // a · b = b · a
  private suspend fun <F> Semiring<F>.semiringMultiplicativeCommutativity(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN, GEN) { a, b ->
      a.combineMultiplicate(b).equalUnderTheLaw(b.combineMultiplicate(a), EQ)
    }

  // 1 · a = a
  private suspend fun <F> Semiring<F>.semiringMultiplicativeLeftIdentity(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN) { A ->
      (one().combineMultiplicate(A)).equalUnderTheLaw(A, EQ)
    }

  // a · 1 = a
  private suspend fun <F> Semiring<F>.semiringMultiplicativeRightIdentity(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN) { A ->
      A.combineMultiplicate(one()).equalUnderTheLaw(A, EQ)
    }

  // a · (b · c) = (a · b) · c
  private suspend fun <F> Semiring<F>.semiringMultiplicativeAssociativity(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN, GEN, GEN) { A, B, C ->
      A.combineMultiplicate(B.combineMultiplicate(C)).equalUnderTheLaw((B.combineMultiplicate(A)).combineMultiplicate(C), EQ)
    }

  // (a + b) · c = a · c + b · c
  private suspend fun <F> Semiring<F>.semiringRightDistributivity(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN, GEN, GEN) { A, B, C ->
      (A.combine(B)).combineMultiplicate(C).equalUnderTheLaw((A.combineMultiplicate(C)).combine(B.combineMultiplicate(C)), EQ)
    }

  // a · (b + c) = a · b + a · c
  private suspend fun <F> Semiring<F>.semiringLeftDistributivity(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN, GEN, GEN) { A, B, C ->
      A.combineMultiplicate(B.combine(C)).equalUnderTheLaw((A.combineMultiplicate(B)).combine(A.combineMultiplicate(C)), EQ)
    }

  // 0 · a = 0
  private suspend fun <F> Semiring<F>.semiringMultiplicativeLeftAbsorption(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN) { A ->
      (zero().combineMultiplicate(A)).equalUnderTheLaw(zero(), EQ)
    }

  // a · 0 = 0
  private suspend fun <F> Semiring<F>.semiringMultiplicativeRightAbsorption(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN) { A ->
      A.combineMultiplicate(zero()).equalUnderTheLaw(zero(), EQ)
    }

  private suspend fun <F> Semiring<F>.timesIsDerived(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN, GEN) { A, B ->
      A.times(B).equalUnderTheLaw(A.combineMultiplicate(B), EQ)
    }

  private suspend fun <F> Semiring<F>.plusIsDerived(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN, GEN) { A, B ->
      A.plus(B).equalUnderTheLaw(A.combine(B), EQ)
    }

  private suspend fun <F> Semiring<F>.maybeCombineAdditionIsDerived(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN, GEN) { A, B ->
      A.maybeCombineAddition(B).equalUnderTheLaw(A.combine(B), EQ)
    }

  private suspend fun <F> Semiring<F>.maybeCombineAdditionLeftNull(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN) { A ->
      null.maybeCombineAddition(A).equalUnderTheLaw(zero(), EQ)
    }

  private suspend fun <F> Semiring<F>.maybeCombineAdditionRightNull(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN) { A ->
      A.maybeCombineAddition(null).equalUnderTheLaw(A, EQ)
    }

  private suspend fun <F> Semiring<F>.maybeCombineAdditionBothNull(EQ: Eq<F>) =
    null.maybeCombineAddition(null).equalUnderTheLaw(zero(), EQ) shouldBe true

  private suspend fun <F> Semiring<F>.maybeCombineMultiplicateIsDerived(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN, GEN) { A, B ->
      A.maybeCombineMultiplicate(B).equalUnderTheLaw(A.combineMultiplicate(B), EQ)
    }

  private suspend fun <F> Semiring<F>.maybeCombineMultiplicateLeftNull(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN) { A ->
      null.maybeCombineMultiplicate(A).equalUnderTheLaw(one(), EQ)
    }

  private suspend fun <F> Semiring<F>.maybeCombineMultiplicateRightNull(GEN: Arb<F>, EQ: Eq<F>) =
    forAll(GEN) { A ->
      A.maybeCombineMultiplicate(null).equalUnderTheLaw(A, EQ)
    }

  private suspend fun <F> Semiring<F>.maybeCombineMultiplicateBothNull(EQ: Eq<F>) =
